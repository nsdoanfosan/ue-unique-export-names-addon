...
`/Game/texture` and material instances based on
...
Packed textures keep a persistent source backup inside the `.blend`. Repeating
Prepare exports the current image, including newer painted or repacked pixels.
Restore recovers the exact original packed bytes and format when content has
not changed. Edited content is retained as a lossless packed image (PNG, or EXR
with channel-preserving alpha for float buffers); the original packed payload
remains available through the image's `ue_unique_packed_original_recovery`
datablock property. Restoring names and paths does not undo later painting.
