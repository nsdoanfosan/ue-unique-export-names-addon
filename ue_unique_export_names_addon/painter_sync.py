...
_painter_export_scene_pointer = 0
...
def _clear_painter_export_sync_state():
    ...
    global _painter_export_scene_pointer
...
def _capture_painter_export_sync_state(low_collection, low_objects, desired, export_coll, scene):
    global _painter_export_sync_state_ready
    global _painter_export_object_signatures
    global _painter_export_low_object_pointers
    global _painter_export_collection_signature
...
def sync_painter_export(scene=None):
    ...
    try:
        ...
        low_collection = baking_low_collection(scene)
        if low_collection is None:
            if not _painter_export_sync_state_ready or _id_pointer(scene) != _painter_export_scene_pointer:
                # Keep ownership independently of the low collection: deleting
                # or unlinking it still requires cleanup in its original scene.
                # A callback for another scene must leave these links intact.
                return {"linked": 0, "unlinked": 0, "desired": 0}
        low_objects = (
